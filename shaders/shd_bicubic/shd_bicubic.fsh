...
void main() {
    vec2 coord = v_vTexcoord / u_vTexel - 0.5;
    vec2 f = fract(coord);
    coord -= f;
    ...
    vec4 sample_0 = texture2D(gm_BaseTexture, offset.xy * u_vTexel);
    vec4 sample_1 = texture2D(gm_BaseTexture, offset.zy * u_vTexel);
    vec4 sample_2 = texture2D(gm_BaseTexture, offset.xw * u_vTexel);
    vec4 sample_3 = texture2D(gm_BaseTexture, offset.zw * u_vTexel);
